...
def send_request(request, dns_url):
    ...
    if request.method() == "GET":
        ...
        result = requests.post(url, json=request.data(), headers=headers)
    elif request.method() == "DELETE":
        result = requests.delete(url, headers=headers)
    if result == None or result.status_code != 200:
        if result != None:
            raise Exception("Failed request, {}".format(result.json()))
        else:
            raise Exception("Failed request")
    return result.json()

def do_add_record_command(args):
    return send_request(dr.AddDNSRecordRequest(args.ip, args.hosts.split()), args.dns_url)

def do_add_hosts_command(args):
    return send_request(dr.AddDNSRecordHostsRequest(args.ip, args.hosts.split()), args.dns_url)

def do_remove_host_command(args):
    return send_request(dr.RemoveDNSRecordHostRequest(args.host), args.dns_url)

def do_remove_record_command(args):
    return send_request(dr.RemoveDNSRecordRequest(args.ip), args.dns_url)

def do_list_records_command(args):
    return send_request(dr.ListDNSRecordsRequest(), args.dns_url)

def do_list_records_by_ip_command(args):
    return send_request(dr.ListDNSRecordsByIPRequest(args.ip), args.dns_url)

def do_clear_records_command(args):
    return send_request(dr.ClearDNSRecordsRequest(), args.dns_url)
